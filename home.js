...
const securityQA = {
    'ما هو الأمن السيبراني': 'الأمن السيبراني هو حماية الأنظمة والشبكات والبرامج من الهجمات الرقمية. هذه الهجمات تهدف عادةً إلى الوصول إلى المعلومات الحساسة أو تغييرها أو إتلافها.',
    'كيف أحمي نفسي من الاختراق': 'لحماية نفسك من الاختراق: 1- استخدم كلمات مرور قوية ومختلفة لكل حساب 2- فعّل المصادقة الثنائية 3- حدّث برامجك باستمرار 4- استخدم برنامج مكافحة فيروسات موثوق 5- كن حذراً من الروابط والمرفقات المشبوهة.',
    'ما هو التصيد الاحتيالي': 'التصيد الاحتيالي هو محاولة خداع المستخدمين للحصول على معلوماتهم الحساسة مثل كلمات المرور وأرقام البطاقات البنكية. عادةً ما يتم ذلك عبر رسائل بريد إلكتروني أو رسائل نصية مزيفة.',
    'كيف أعرف أن جهازي مخترق': 'علامات اختراق الجهاز تشمل: 1- بطء غير عادي في الأداء 2- تغييرات غير مصرح بها 3- ظهور برامج غريبة 4- نشاط شبكة غير طبيعي 5- تعطيل برامج الحماية.',
    'ما هي المصادقة الثنائية': 'المصادقة الثنائية هي طبقة أمان إضافية تتطلب عاملين للتحقق من هويتك: كلمة المرور ورمز إضافي يرسل عادةً إلى هاتفك. هذا يجعل اختراق حسابك أكثر صعوبة.',
    'default': 'عذراً، لا أستطيع الإجابة على هذا السؤال حالياً. حاول طرح سؤال آخر متعلق بالأمن السيبراني.'
};

// Hugging Face API Token
const HF_API_TOKEN = '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';
...
async function getAIResponse(question) {
    try {
        ...
        return result[0]?.generated_text || securityQA['default'];
    } catch (error) {
        console.error('AI Error:', error);
        // استخدام الردود المحلية كنظام احتياطي
        return findAnswer(question);
    }
}

// تحديث دالة sendMessage لاستخدام الذكاء الاصطناعي
async function sendMessage() {
    ...
    try {
        ...
        removeTypingIndicator();
        const fallbackResponse = findAnswer(message);
...
function findAnswer(question) {
    ...
    // البحث عن أقرب إجابة
    for (let key in securityQA) {
        if (question.includes(key.toLowerCase())) {
            return securityQA[key];
        }
    }
    
    // إرجاع الإجابة الافتراضية إذا لم يتم العثور على تطابق
    return securityQA['default'];
}
